...
case "${#MUSTACHE_DEBUG}" in (0) exec 3>/dev/null;; (*) exec 3>&2;; esac
...
_mustache_sed() {
	_M_NEWLINE="$(printf \\012_)"; _M_NEWLINE="${_M_NEWLINE%_}"
	set +e
	sed -r <"/dev/null" >"/dev/null" 2>&1
    _M_STATUS="$?"
	set -e
	case "$_M_STATUS" in (1) sed -E "s/./&\\$_M_NEWLINE/g; s/\\\\/\\\\\\\\/g"
	;;(*) sed -r "s/./&\\n/g; s/\\\\/\\\\\\\\/g"
	;;esac
}
